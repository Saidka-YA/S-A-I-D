...
string encryptBifid(const string& plaintext) {
    ...
    // 1. Переводим исходный текст в Hex
    string hexStr;
    ...
    // Извлекаем координаты строк и столбцов для каждой hex-цифры
    ...
    // Логика Бифида: объединяем строки и столбцы
    vector<int> combined = rows;
    combined.insert(combined.end(), cols.begin(), cols.end());

    // Формируем зашифрованную Hex-строку
    string cipherHex;
    ...
    return cipherHex; // Возвращаем как текст 
}
...
string decryptBifid(const string& ciphertext) {
    ...
    // Извлекаем координаты из зашифрованной Hex-строки
    ...
    // Разделяем массив координат обратно на две половины
    size_t half = flatCoords.size() / 2;
    
    // Восстанавливаем оригинальную Hex-строку исходного текста
    string origHex;
    ...
    // Переводим Hex-строку обратно в нормальные байты (символы UTF-8)
    string plaintext;
